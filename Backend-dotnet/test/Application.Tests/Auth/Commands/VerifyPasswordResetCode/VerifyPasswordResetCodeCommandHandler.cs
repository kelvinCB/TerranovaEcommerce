...
public sealed class VerifyPasswordResetCodeCommandHandlerTests
{
    ...
    public async Task Handle_ShouldReturnTrueAndConsumeVerification_WhenCodeIsValid()
    {
        ...
        mockUserVerificationRepository
            .Setup(x => x.GetByUserIdAndTypeAsync(user.Id, UserVerificationPurpose.PasswordReset, It.IsAny<CancellationToken>()))
        ...
        mockUserVerificationRepository.Verify(x => x.GetByUserIdAndTypeAsync(user.Id, UserVerificationPurpose.PasswordReset, CancellationToken.None), Times.Once);
    ...
    public async Task Handle_ShouldPropagateCancellationToken()
    {
        ...
        mockUserVerificationRepository.Setup(x => x.GetByUserIdAndTypeAsync(user.Id, UserVerificationPurpose.PasswordReset, token)).ReturnsAsync(userVerification);
        mockDateTimeProvider.SetupGet(x => x.Timestamp).Returns(Timestamp);
        ...
        mockUserVerificationRepository.Verify(x => x.GetByUserIdAndTypeAsync(user.Id, UserVerificationPurpose.PasswordReset, token), Times.Once);
    ...
    public async Task Handle_ShouldReturnFalse_WhenUserDoesNotExist()
    {
        ...
        Assert.False(result);
        mockUserVerificationRepository.Verify(x => x.GetByUserIdAndTypeAsync(It.IsAny<Ulid>(), It.IsAny<UserVerificationPurpose>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Handle_ShouldReturnFalse_WhenCodeDoesNotMatch()
    {
        ...
        mockUserVerificationRepository.Setup(x => x.GetByUserIdAndTypeAsync(user.Id, UserVerificationPurpose.PasswordReset, It.IsAny<CancellationToken>())).ReturnsAsync(userVerification);
        mockDateTimeProvider.SetupGet(x => x.Timestamp).Returns(Timestamp);
